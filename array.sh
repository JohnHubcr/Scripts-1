...
echo "--------------------------------------------------------------------"
echo "|                        Array v1.0 ~ b33f                         |"
echo "|                      -Generate Byte-Array-                       |"
echo "--------------------------------------------------------------------"
echo "| USAGE: ./array.sh -b [Badchars]                                  |"
echo "|                                                                  |"
echo "| REQUIRED                                                         |"
echo "|        -b  The format should be like \xAA\xBB\xCC                |"
echo "--------------------------------------------------------------------"
...
#256 possible bytes
#######################################
BYTES='00,01,02,03,04,05,06,07,08,09,0A,0B,0C,0D,0E,0F,10,11,12,13,14,15,
...
#######################################
echo $BYTES |tr -d ' ' |fold -w 3 >> /tmp/bytearray.txt

#EGREP Parsing Filter
#######################################
echo $BAD |sed 's/[\"x]//g' |fold -w 2 |sed 's/$/|/' |tr -d '\n' |sed -e 's/|$//' |tr [a-z] [A-Z] >> /tmp/filter.txt

#Parse badcharacters
#######################################
echo "[>] Badchars:" $BAD| sed 's/x/\\x/g'
echo ""
echo "[>] Byte-Array:"
for FILTER in $(cat /tmp/filter.txt); do
cat /tmp/bytearray.txt |egrep -v "$FILTER" |sed 's/,//' |sed 's/^/\\x/' |tr -d ' ' |tr -d '\n' |fold -w 84 |sed 's/$/"/' |sed 's/^/"/'
done
echo ""

rm /tmp/filter.txt
rm /tmp/bytearray.txt
