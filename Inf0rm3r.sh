...
echo 'Th3 Inf0rm3r v0.2' | grep --color -i 'Th3 Inf0rm3r v0.2'
...
ifconfig -a | grep --color -E 'eth||lo||tun||P-t-P||wlan||vmnet||inet addr||inet6 addr||Bcast||Mask||Link encap||HWaddr' 2> /dev/null
}
...
which gcc nc nmap ncat lynx curl wget php perl python ruby /etc/valiases | grep --color -i -E '/usr/bin|/etc/valiases|/bin' 2> /dev/null
echo ''
echo 'Grabbing Local Tools Version Info: ' | grep --color 'Grabbing Local Tools Version Info'
gcc --version | grep --color 'gcc' 2> /dev/null
mysql --version | grep --color -i -E 'mysql||Ver' 2> /dev/null
perl -v | grep --color -i -E 'This is perl|version|subversion|built for' 2> /dev/null
